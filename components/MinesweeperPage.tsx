...
import { ArrowPathIcon, FireIcon, FlagIcon, SeymourIcon } from './icons';
...
const MinesweeperPage: React.FC<MinesweeperPageProps> = ({ onBackToHub, onReturnToFeeds }) => {
  ...
  const reset = () => { setGameState('IDLE'); setGrid(null); setTime(0); stopTimer(); };
        ...
        <div className="max-w-2xl w-full flex flex-col gap-6">
            <header className="flex justify-between items-center bg-zinc-900/50 p-6 rounded-[2rem] border-2 border-white/5 backdrop-blur-xl">
                ...
                <div className="flex gap-4">
                  <div className="bg-black/40 px-4 py-2 rounded-xl text-center">
                    <span className="text-[8px] font-black text-flesh-500 uppercase block">Alerts</span>
                    <span className="text-xl font-black italic font-mono text-flesh-400">{flagsLeft}</span>
                  </div>
                   <div className="bg-black/40 px-4 py-2 rounded-xl text-center">
            ...
            <div className="bg-zinc-900 p-2 rounded-[2.5rem] border-4 border-zinc-800 shadow-2xl overflow-hidden">
                <div 
                  ...
                  style={{ gridTemplateColumns: `repeat(${settings[difficulty].cols}, 1fr)` }}
                >
                  {grid?.map((row, r) => row.map((cell, c) => {
                    ...
                    return (
                        <div 
                            ...
                            className={`aspect-square flex items-center justify-center text-sm sm:text-base font-black rounded-lg transition-all cursor-pointer
                                ${cell.isRevealed ? (cell.isMine ? 'bg-flesh-600 scale-90' : 'bg-black/30') : 'bg-zinc-800 hover:bg-zinc-700 shadow-inner'}
                                ${cell.isFlagged ? 'bg-yellow-500/20' : ''}
                            `}
                        >
                            {cell.isRevealed ? (cell.isMine ? '☣️' : cell.adjacentMines || '') : (cell.isFlagged ? '🚩' : '')}
                            {cell.isRevealed && !cell.isMine && cell.adjacentMines > 0 && (
                                <span className={colors[cell.adjacentMines]}>{cell.adjacentMines}</span>
                            )}
                        </div>
                    )
            ...
            <div className="flex flex-wrap justify-center gap-4">
                <button onClick={() => setIsFlagMode(!isFlagMode)} className={`px-6 py-3 rounded-2xl font-black uppercase text-xs italic tracking-widest border-2 transition-all ${isFlagMode ? 'bg-flesh-600 border-flesh-400 text-white shadow-[0_0_20px_rgba(236,72,153,0.3)]' : 'bg-zinc-900 border-white/5 text-zinc-500'}`}>
                    Flag Mode: {isFlagMode ? 'ON' : 'OFF'}
                </button>
                <div className="flex bg-zinc-900 p-1 rounded-2xl border-2 border-white/5">
                   {(['Easy', 'Medium', 'Hard'] as Difficulty[]).map(d => (
                     <button key={d} onClick={() => { setDifficulty(d); reset(); }} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase italic transition-all ${difficulty === d ? 'bg-plant-600 text-black' : 'text-zinc-500 hover:text-white'}`}>{d}</button>
                   ))}
                </div>
                <button onClick={onBackToHub} className="px-6 py-3 bg-zinc-900 border-2 border-white/5 rounded-2xl font-black uppercase text-xs italic tracking-widest text-zinc-500 hover:text-white">Exit Pit</button>
            </div>
        </div>

        {gameState !== 'PLAYING' && gameState !== 'IDLE' && (
            <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-50 flex items-center justify-center p-6">
                <div className="max-w-sm w-full bg-zinc-900 p-12 rounded-[3rem] border-4 border-plant-500 text-center shadow-[0_0_100px_rgba(34,197,94,0.3)]">
                    <h2 className={`text-6xl font-black italic uppercase tracking-tighter mb-4 ${gameState === 'WON' ? 'text-plant-500' : 'text-flesh-500'}`}>
                        {gameState === 'WON' ? 'CLEAN!' : 'BURST!'}
                    </h2>
                    <p className="text-zinc-500 font-bold uppercase tracking-widest mb-10">{gameState === 'WON' ? 'Pods neutralized.' : 'Spores released.'}</p>
                    <button onClick={reset} className="w-full py-5 bg-plant-600 text-black font-black text-xl italic uppercase rounded-full hover:scale-105 transition-transform">Incubate Again</button>
                </div>
            </div>
        )}
    </div>
