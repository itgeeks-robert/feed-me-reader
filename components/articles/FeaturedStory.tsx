            ...
            <ImageWithProxy
                ...
                fallback={
                    <div className="w-full h-full static-noise flex items-center justify-center">
                        <span className="text-[10px] md:text-xs font-mono text-white/20 tracking-[0.5em] animate-pulse uppercase">Signal Lost</span>
                    </div>
                }
            />
    ...
    return (
        <div className={`group grid grid-cols-1 md:grid-cols-[1fr,1.3fr] bg-void-900 border-2 border-zinc-800 text-white shadow-[12px_12px_0px_black] overflow-hidden min-h-[250px] md:min-h-[350px] transition-all duration-500 hover:translate-x-[-4px] hover:translate-y-[-4px] ${isRead ? 'opacity-50 grayscale' : ''}`}>
            ...
            <div className="relative p-6 md:p-10 flex flex-col justify-center bg-void-950/40 backdrop-blur-sm border-l-8 border-pulse-500">
                ...
                <p className="text-xs md:text-base text-zinc-500 font-mono line-clamp-3 mb-8 md:mb-12 leading-relaxed uppercase tracking-wider">{article.snippet}</p>
                ...
                <div className="flex flex-wrap items-center gap-6 md:gap-8 mt-auto">
                    ...
                    <a href={article.link} target="_blank" rel="noopener noreferrer" onClick={onMarkAsRead} className="text-[10px] md:text-xs font-black uppercase tracking-widest text-zinc-600 hover:text-white transition-colors font-mono">
