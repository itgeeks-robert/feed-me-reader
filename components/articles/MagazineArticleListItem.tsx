        ...
        <div className={`group relative flex flex-col bg-void-900 border-2 border-zinc-300 dark:border-zinc-800 shadow-[6px_6px_0px_black] dark:shadow-[6px_6px_0px_#000] hover:border-pulse-500/50 hover:translate-x-[-1px] hover:translate-y-[-1px] transition-all duration-300 overflow-hidden h-full ${isRead ? 'opacity-40 grayscale' : ''}`}>
            ...
            <div className="relative aspect-[16/10] w-full overflow-hidden border-b-2 border-zinc-300 dark:border-zinc-800 bg-black shrink-0">
                <ImageWithProxy
                    ...
                    fallback={
                        <div className="w-full h-full static-noise flex flex-col items-center justify-center p-4">
                            <span className="text-[10px] font-black text-signal-500 tracking-[0.2em] uppercase italic animate-pulse text-center">
                ...
                <div className="absolute top-2 right-2 flex items-center gap-1.5 px-2 py-0.5 bg-black/60 backdrop-blur-md border border-white/10 rounded-sm">
                    <div className={`w-1.5 h-1.5 rounded-full ${reconstructedSrc && !article.imageUrl ? 'bg-amber-500 animate-pulse' : 'bg-pulse-500'}`} />
                    <span className="text-[9px] font-mono font-black text-zinc-300 uppercase tracking-tighter">
            ...
            <div className="p-4 md:p-5 flex flex-col flex-grow bg-void-950/20">
                <div className="flex items-center gap-2 mb-3">
                    <div className="p-0.5 bg-void-950 border border-zinc-300 dark:border-zinc-800">
                        <SmartFeedIcon iconUrl={iconUrl || ''} feedTitle={article.source} sourceType={sourceType} className="w-4 h-4 grayscale group-hover:grayscale-0 transition-all" />
                    </div>
                    <span className="text-[9px] md:text-[10px] font-mono font-black text-pulse-600 dark:text-pulse-500 uppercase tracking-widest truncate">{article.source}</span>
                </div>
                
                <h3 className="font-black text-zinc-900 dark:text-white italic uppercase tracking-tighter line-clamp-3 leading-tight mb-3 font-horror text-sm md:text-lg lg:text-xl group-hover:text-pulse-600 dark:group-hover:text-pulse-500 transition-colors">
                    {article.title}
                </h3>
                
                <p className="text-[11px] md:text-sm text-zinc-800 dark:text-zinc-300 font-mono line-clamp-3 md:line-clamp-4 uppercase tracking-tight mb-6 leading-relaxed">
                    {article.snippet}
                </p>

                <div className="mt-auto flex flex-col gap-3.5 border-t border-zinc-300 dark:border-zinc-800/40 pt-4">
                    <button 
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); onReadHere(); }} 
                        className="flex items-center justify-center gap-2 w-full py-3 bg-pulse-500 text-white font-black uppercase italic tracking-widest shadow-[3px_3px_0px_black] dark:shadow-[3px_3px_0px_white] active:translate-x-0.5 active:translate-y-0.5 active:shadow-none transition-all text-xs"
                    >
                    ...
                    <div className="flex items-center justify-between">
                        <span className="text-[10px] font-mono font-black text-zinc-600 dark:text-zinc-500 uppercase tracking-tighter">{timeAgo(article.publishedDate)}</span>
                        <button 
                            onClick={(e) => { e.preventDefault(); e.stopPropagation(); onReadExternal(); }} 
                            className="text-[10px] font-black uppercase text-zinc-600 dark:text-zinc-500 hover:text-terminal transition-colors underline decoration-dotted underline-offset-2 flex items-center gap-1.5"
                        >
