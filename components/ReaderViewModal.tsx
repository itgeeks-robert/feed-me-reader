...
const ReaderViewModal: React.FC<ReaderViewModalProps> = ({ article, onClose, onMarkAsRead }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center backdrop-blur-sm" onClick={onClose} role="dialog" aria-modal="true">
            <div className="bg-zinc-900 rounded-lg shadow-xl w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <header className="flex items-center justify-between p-4 border-b border-zinc-800 flex-shrink-0">
                    <div className="flex-grow min-w-0">
                        {isLoading ? (
                            ...
                            <h2 className="text-lg font-bold text-white truncate">{parsedContent?.title || article.title}</h2>
                        )}
                        <a href={article.link} target="_blank" rel="noopener noreferrer" className="text-xs text-zinc-400 hover:text-orange-400 truncate block">
                            {article.source}
                        </a>
                    </div>
                    <button onClick={onClose} className="ml-4 p-2 rounded-full text-zinc-400 hover:bg-zinc-700 flex-shrink-0" aria-label="Close reader view">
                        <XIcon className="w-6 h-6" />
                    </button>
                </header>
                
                <div className="p-6 md:p-8 overflow-y-auto flex-grow prose prose-invert prose-zinc max-w-none prose-p:text-zinc-300 prose-headings:text-white prose-a:text-orange-400 hover:prose-a:text-orange-300 prose-strong:text-white prose-img:rounded-lg">
                    {isLoading && (
                        <div className="flex items-center justify-center h-full">
                             <div className="flex flex-col items-center gap-4">
                                <ArrowPathIcon className="w-8 h-8 text-zinc-500 animate-spin" />
                                <span className="text-zinc-400">Fetching article...</span>
                            </div>
                        </div>
                    )}
                    {error && (
