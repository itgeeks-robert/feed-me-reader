...
const VHSCard: React.FC<{ game: GameInfo; onPlay: () => void; isHighlighted?: boolean }> = ({ game, onPlay, isHighlighted }) => {
    ...
    return (
        <div className={`group relative bg-void-900 border-2 ${isHighlighted ? 'border-cyan-400 shadow-[0_0_30px_rgba(34,211,238,0.2)]' : 'border-zinc-800'} hover:border-cyan-400 transition-all duration-300 h-[480px] shadow-[10px_10px_0px_black] hover:translate-x-[-2px] hover:translate-y-[-2px] flex flex-col`}>
            {isHighlighted && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-cyan-400 text-black px-4 py-1 rounded-full text-[7px] font-black uppercase tracking-[0.3em] z-20 shadow-[0_0_15px_rgba(34,211,238,0.5)] animate-bounce">
                    High_Voltage
                </div>
            )}
            ...
            <div className="h-40 w-full bg-void-950 flex items-center justify-center relative overflow-hidden border-b-2 border-zinc-800">
                ...
                <div className={`opacity-20 group-hover:opacity-60 transition-all duration-700 ${isHighlighted ? 'text-cyan-400 scale-125 opacity-40' : 'text-zinc-600'} scale-110`}>
                    {React.cloneElement(game.icon, { className: "w-24 h-24" })}
                </div>
                <div className={`absolute bottom-0 left-0 right-0 h-1 ${isHighlighted ? 'bg-cyan-400' : 'bg-pulse-500/30'} animate-pulse`}></div>
            </div>

            <div className="p-6 flex flex-col flex-grow justify-between bg-void-950/20">
                <div className="text-center">
                    <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter mb-1 group-hover:text-cyan-400 transition-colors">{game.title}</h3>
                ...
                <div className="flex flex-col items-center gap-5 mt-6">
                     ...
                     <button 
                        ...
                        className={`w-[85%] py-5 bg-white text-black font-black uppercase italic text-sm tracking-[0.2em] rounded-2xl shadow-[6px_6px_0px_#e11d48] transition-all hover:bg-cyan-400 active:scale-90 active:bg-cyan-600 flex items-center justify-center gap-3 group/btn relative overflow-hidden`}
                     >
                ...
                <header className="mb-20 flex flex-col lg:flex-row lg:items-center justify-between gap-12 border-b-2 border-pulse-500/20 pb-12">
                    <div className="flex items-center gap-8">
                        ...
                        <div>
                            ...
                            <p className="text-pulse-500 font-bold tracking-[0.8em] uppercase text-[10px] md:text-xs mt-4 font-mono">Terminal v1.8.4 - Simulation Hub</p>
                        </div>
                    </div>
