...
export const DailyRationPage: React.FC<DailyRationPageProps> = ({ feeds, fertilizer, onComplete, onEnterArcade, onSelectGame }) => {
...
export const DailyUplinkPage: React.FC<DailyUplinkPageProps> = ({ feeds, uptime, onComplete, onEnterArcade, onSelectGame }) => {
    const [articles, setArticles] = useState<Article[]>([]);
    const [loading, setLoading] = useState(true);
    const [readerArticle, setReaderArticle] = useState<Article | null>(null);
                    ...
                    <aside className="space-y-12 md:space-y-20">
                         <div className={`p-8 md:p-10 border-2 transition-all relative overflow-hidden ${isCipherDone ? 'bg-void-900 border-zinc-800 opacity-60' : 'bg-void-900 border-emerald-500 shadow-[10px_10px_0px_#065f46]'}`}>
                            ...
                            {isCipherDone ? (
                                ...
                                <button onClick={() => onSelectGame('cipher_core')} className="w-full py-4 md:py-6 bg-emerald-500 text-white font-black uppercase italic rounded-none hover:bg-white hover:text-black transition-all shadow-[4px_4px_0px_white] text-xs md:text-sm">Intercept Now</button>
                            )}
                        </div>
